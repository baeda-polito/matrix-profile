...
# useful paths
path_to_data = 'Ventilation_Usecase' + os.sep + 'Ventilation_Data' + os.sep
path_to_figures = 'Ventilation_Usecase' + os.sep
color_palette = 'viridis'

# Read data for 3 ventilation units, each unit measures CO2 content at 15 minute intervals.
data = [
    pd.read_csv(path_to_data + "ventilation0.csv", index_col=0, header=None, parse_dates=[0],
                names=["Time", "CO2"]).iloc[:, 0],
    pd.read_csv(path_to_data + "ventilation1.csv", index_col=0, header=None, parse_dates=[0],
                names=["Time", "CO2"]).iloc[:, 0],
    pd.read_csv(path_to_data + "ventilation2.csv", index_col=0, header=None, parse_dates=[0],
                names=["Time", "CO2"]).iloc[:, 0]
]
# List of daily dates
...
for i in range(3):
    im = ax[i, 0].imshow(cmps_morning[i].distance_matrix, cmap="viridis", vmin=0, vmax=6.8, extent=extents,
                         origin="lower")
    im = ax[i, 1].imshow(cmps_noon[i].distance_matrix, cmap="viridis", vmin=0, vmax=6.8, extent=extents, origin="lower")
    im = ax[i, 2].imshow(cmps_evening[i].distance_matrix, cmap="viridis", vmin=0, vmax=6.8, extent=extents,
                         origin="lower")

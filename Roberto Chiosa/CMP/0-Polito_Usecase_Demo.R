...
figma_palette <- c("#5EE393", "#0074FF", "#FF0005")
...
df <- read.csv(file.path(dirname(dirname(getwd())), 
  ...
  # add subsequences
  mutate(tag = ifelse(
    timestamp >= "2015-05-03 00:00:00" & timestamp <= "2015-05-04 00:00:00", 
    "Subsequence 1",
    ifelse(
      timestamp >= "2015-05-17 00:00:00" & timestamp <= "2015-05-18 00:00:00",
      "Subsequence 2",
      ifelse(
        timestamp >= "2015-06-04 15:00:00" & timestamp <= "2015-06-05 15:00:00",
        "Subsequence 3", NA
      )
    )
  ),

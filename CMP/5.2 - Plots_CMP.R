...
df_context_decoder <- read.csv(file.path("Polito_Usecase","data", "contexts.csv"))
...
for (i in 1:length(context_folder_vector)) {
  ...
  context_folder <- context_folder_vector[i]
  
  # decode context string
  context_string_small <- gsub("Polito_Usecase/data/", "", context_folder)
  context_string <- df_context_decoder$context_string[df_context_decoder$context_string_small == context_string_small]
  
  # list files of context
  folder_files <- list.files( context_folder )
  
  # remove not useful files
  cluster_files <-   folder_files[!folder_files %in% c("plot_cmp_full.csv")]
  ...
  colnames(df) <- as.Date(read.csv(file.path("Polito_Usecase", "data", "group_cluster.csv"), sep = ',', header = T)[,1])
  rownames(df) <- as.Date(read.csv(file.path("Polito_Usecase", "data", "group_cluster.csv"), sep = ',', header = T)[,1])
  ...
  legend_breaks <- ggeffects::pretty_range(0:round(max(df_long$values,  na.rm = T)), length = 6)
  ...
  plot_full <- df_long %>%
    ...
    coord_fixed(ratio = 1) +
    labs( title = paste("CMP for context",i),
  ...
  dev.new()
  plot_full
  ggsave(filename = file.path(gsub("data", "figures", context_folder_vector[i]),  "cmp_full.jpg"),
    width = 7, height = 7, dpi = dpi ,  bg = background_fill)
  ...
  for (j in 1:length(cluster_files)) {
    ...
    df1 <-  read.csv(file.path(context_folder, cluster_files[j]), sep = ',', header = F)
    ...
    legend_breaks <- ggeffects::pretty_range(0:round(max(df_long1$values,  na.rm = T)), length = 6)
    xy_breaks <- ggeffects::pretty_range(1:dim(df1)[1], length = 8)  
    ...
    plot_list[[j]] <- df_long1 %>%
      ...
      labs( title = plot_title, subtitle = " ",
        x = "" , y = "", fill = "") +                   # axis label
      ggplot2::theme(
        text = element_text(family = font_family),
        plot.title = element_text(hjust = 0.5, size = fontsize_large, margin = margin(t = 0, r = 0, b = 0, l = 0), ),
        ...
        legend.text = element_text(size = fontsize_small),
        legend.position = "right",                     # legend position on the top of the graph
        legend.key.height = unit(1.5, "cm"),          # size of legend keys, tacche legenda
        legend.key.width = unit(0.4, "cm"),
        legend.direction = "vertical",             # layout of items in legends
        legend.box = "vertical",                   # arrangement of multiple legends
        legend.title = element_blank(),      # title of legend (inherits from title)
  ...
  dev.new()
  ggarrange(plotlist = plot_list, nrow = 1, align = c("h"))
  ggsave(filename = file.path(gsub("data", "figures", context_folder_vector[i]),  "cmp_groups_horizontal.jpg"),
    width = 20, height = 5.5, dpi = dpi ,  bg = background_fill)
  dev.off()
  
  # version 2 compact
  dev.new()
  ggarrange(plotlist = plot_list)
  ggsave(filename = file.path(gsub("data", "figures", context_folder_vector[i]),  "cmp_groups_compact.jpg"),

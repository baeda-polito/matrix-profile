...
extra.calendarHeat <- function(dates,
  ...
  # Reshape data  ------------------------------------------------------------------
  transformdata <-
    function(dates,  values, pvalues, date.form = "%Y-%m-%d", ...) {
      ...
      caldat <-
        data.frame(
          date.seq =  seq(min.date, max.date, by = "days") ,
          value = NA,
          pvalue = NA
        )
      dates <- as.Date(dates)
      ...
      caldat$pvalue.n <-
        round(caldat$pvalue / max(caldat$pvalue, na.rm = T), 2)
      caldat
  ...
  calendar.theme <- list(
    xlab = NULL,
    ylab = NULL,
    axis.text = list(fontfamily = fontfamily),

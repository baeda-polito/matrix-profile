...
# load a CMP
group_cmp <-  read.csv(file.path("Polito_Usecase", "data", "ctx_from05_15_to06_15_m02_30", "plot_cmp_Cluster_3.csv"), sep = ',', header = F)

group_cmp_vector <- group_cmp %>% 
  summarise(across(everything(), mean, na.rm = T)) %>% # do the median by column
  t() %>% # transpose
  as.data.frame() %>% # back to df 
  rename(values = V1) %>% # rename variable
  arrange(desc(values)) %>% # arrange in deschending order
  mutate(
    index = seq(1:dim(group_cmp)[1]), # add index to keep track
    z = (values-mean(values))/sd(values)  # znormalize
  )


# extract some plot variables useful for plots
dist_min <- min(group_cmp_vector$values, na.rm = T) # minimum value of distance
dist_max <- max(group_cmp_vector$values, na.rm = T) # minimum value of distance
dist_max <- round(dist_max/100)*100 # round to closest 100 for axis
dist_IQR <- max(group_cmp_vector$values, na.rm = T) # get wisker upper
...
plot_boxplot <- group_cmp_vector %>%
  ...
  stat_boxplot(geom ='errorbar', width = 0.6, color = figma_blue) +
  geom_boxplot(
    width = 0.6, 
...
plot_gesd <- ggplot() +
  ...
  geom_line(data = group_cmp_vector, 
    na.rm = T, color = figma_blue, 
    aes(x = index, y=values)) +
  geom_point(data = group_cmp_vector %>% filter(values>200), 
    aes(x = index, y=values), 
    na.rm = T,
    color = outlier_color) +
  scale_x_continuous( expand = c(0,0), limits = c(0,length(group_cmp_vector$values)))+

...
plot_clusters <- function(df_power, df_power_wide){
  df_power <- merge.data.frame(df_power, df_power_wide[c("Date", "cluster")]) 
  centr <- ddply(df_power, c("cluster","Time"), summarise, Total_Power = mean(Total_Power)) 
...
df_calendar <-  read.csv(file.path(getwd(),"Polito_Usecase", "data",  "polito_raw.csv"), sep = ',', dec = ".") %>%
  select(Date, Day_Type, Holiday) %>%
  unique()

df_power <-  read.csv(file.path(getwd(),"Polito_Usecase", "data",  "polito_raw.csv"), sep = ',', dec = ".") %>%
  dplyr::select(Date, Time, Total_Power) 

df_power_wide <- pivot_wider(df_power, names_from = "Time", values_from = "Total_Power")
...
plot
ggsave(filename = file.path("Polito_Usecase", "figures", "groups_clusters_part_1_0.jpg"), 
...
################# fix clusters
# move sunday in 1
df_power_wide <- mutate(df_power_wide, cluster = ifelse(Day_Type == 7, 1, cluster))
# move saturday in 3
df_power_wide <- mutate(df_power_wide, cluster = ifelse(Day_Type == 6 & cluster != 1, 3, cluster))
# merge clusters
df_power_wide <- mutate(df_power_wide, cluster = ifelse(cluster == 5 | cluster == 6, 4, cluster))

# plot horizontal labeled
dev.new()
plot <- plot_clusters(df_power, df_power_wide)
plot
ggsave(filename = file.path("Polito_Usecase", "figures", "groups_clusters_part_1_1.jpg"), 
  width = 10, height = 3, dpi = dpi,  bg = background_fill)
dev.off()
...
Nb_res <- NbClust(cluster_data, diss = diss_matrix, distance = NULL, min.nc = 2, max.nc = 8, method = "kmeans", index = "silhouette")
n_clusters <- length(unique(Nb_res$Best.partition))
...
plot <- plot_clusters(df_power, df_power_wide)
plot
ggsave(filename = file.path("Polito_Usecase", "figures", "groups_clusters_part_2_0.jpg"), 
  width = 10, height = 3, dpi = dpi,  bg = background_fill)
dev.off()
...
plot
ggsave(filename = file.path("Polito_Usecase", "figures", "groups_clusters_part_2_1.jpg"), 
  width = 10, height = 3, dpi = dpi,  bg = background_fill)
dev.off()
...
group_cluster <- data.table(
  timestamp = df_power_wide_all$Date,

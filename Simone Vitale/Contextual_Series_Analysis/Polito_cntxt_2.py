...
m = int(2.5*4.0)# 02:30 hours

context_2= GeneralStaticManager(
    [range(x*96+16, x*96+24) for x in range(data_days.size)]) # daily from 6:00u-8:00u
...
cmp_2.append(calc.add_consumer([0], ContextualMatrixProfile(context_2)))
...
for ii in range(1,7):
 ...
 cluster_cmp = cmp_2[0].distance_matrix[:, cluster][cluster, :]
 ...
 cmp_ad_score = np.zeros(len(cmp_2[0].distance_matrix)) * np.nan

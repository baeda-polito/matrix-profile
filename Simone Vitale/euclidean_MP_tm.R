...
df <- read.csv( "./euclidean_MP/data/df_two_months.csv")

mp_euclidean_tm <- read.csv(file = "./euclidean_MP/data/mp_euclidean_tm.csv",
                         col.names = c("row","mp", "pi", "lpi", "rpi")) %>%
  dplyr::mutate(row = row+1)

# create a MP class
mp_euclidean_tm$row <- NULL
mp_euclidean_tm <- as.list(mp_euclidean_tm)
mp_euclidean_tm$mp <- as.matrix(mp_euclidean_tm$mp)
mp_euclidean_tm$pi <- as.matrix(mp_euclidean_tm$pi)
mp_euclidean_tm$lpi <- as.matrix(mp_euclidean_tm$lpi)
mp_euclidean_tm$rpi <- as.matrix(mp_euclidean_tm$rpi)
mp_euclidean_tm$w <- 96
mp_euclidean_tm$ez <- 0.5
mp_euclidean_tm$data <- list(as.matrix( df$Power_total ))
class(mp_euclidean_tm) <- "MatrixProfile"
...
df_mp_euclidean <- as.data.frame(mp_euclidean_tm[['mp']])
...
p_mp_euclidean <- ggplot()+
  ...
  geom_point(data= df_mp_euclidean, aes(x=X[discords_idx_nn==1], y=mp[discords_idx_nn==1]),colour='blue')+
  geom_point(data= df_mp_euclidean, aes(x=X[discords_idx==1], y=mp[discords_idx==1]),colour='green')+
